...
module.exports = {
	class: L.Control.extend({
		...
		_geocode: function(event) {
			...
			this.options.geocoder.geocode(this._input.value+ ", portugal", this._geocodeResult, this);
...
module.exports = {
	class: L.Class.extend({
		...
		reverse: function(location, scale, cb, context) {
			Util.jsonp('http://dev.virtualearth.net/REST/v1/Locations/' + location.lat + ',' + location.lng, {
				key : this.key
			}, function(data) {
				var results = [];
				for (var i = data.resourceSets[0].resources.length - 1; i >= 0; i--) {
...
module.exports = {
	class: L.Class.extend({
		options: {
			serviceUrl: 'http://www.mapquestapi.com/geocoding/v1'
		},
