...
exports.register = function(server, options, next){
    ...
    server.route({
        ...
        config: {
            handler: {
                directory: { 
                    path: Path.join(Config.get("rootDir"), "lib/client/rc-app"),
                    index: false,
                    listing: false,
                    showHidden: false
                }
    ...
    server.route({
        ...
        config: {
            handler: {
                directory: { 
                    path: Path.join(Config.get("rootDir"), "lib/client/rc-events"),
                    index: false,
                    listing: false,
                    showHidden: false
                }
    ...
    server.route({
        ...
        config: {
            handler: {
                directory: { 
                    path: Path.join(Config.get("rootDir"), "lib/client/rc-dashboard"),
                    index: false,
                    listing: false,
                    showHidden: false
                }
    ...
    server.route({
        ...
        config: {
            handler: {
                directory: { 
                    path: Path.join(Config.get("rootDir"), "lib/client/static"),
                    index: false,
                    listing: false,
                    showHidden: false
                }
    ...
    server.route({
        ...
        config: {
            handler: {
                file: Path.join(Config.get("rootDir"), "lib/client/static/_images/favicon.ico"),
            },
            cache: {
                privacy: "public",
                expiresIn: 3600000
