...
var MapM = Backbone.Model.extend({
    initialize: function(){
        ...
        this.updateInVisibleBounds = _.debounce(function(){

            var model, coordinates, point;
            for(var i=0, l=initiativesC.models.length; i<l; i++){

                model = initiativesC.models[i];
                coordinates = model.attributes.geoJson.geometry.coordinates;
                point = this.attributes.leafletMap.latLngToContainerPoint(
                ...
                if(RC.visibleBounds.contains(point)){
                    model.attributes.inVisibleBounds = true;
                }
                else{
                    model.attributes.inVisibleBounds = false;
                }
            }
    ...
    addZoomControl: function(options) {
        ...
        this.attributes.leafletMap.addControl(zoomControl);
    },
    ...
    addScaleControl: function(options) {
        ...
        this.attributes.leafletMap.addControl(scaleControl);
    },
    ...
    addMapEvents: function(){
        ...
        leafletMap.on("click", function(e){
            ...
            console.log(this.attributes.leafletMap.containerPointToLatLng(e.containerPoint))
...
//debugger;
        var leafletMap = this.attributes.leafletMap;
        var zoom = leafletMap.getZoom(), center = leafletMap.getCenter();

        if(this.attributes.zoom !== zoom){
            console.log("mapIV.updateModel (zoom) @ " + Date.now());
            this.set("zoom", zoom);
        }

        if(this.attributes.center.lat !== center.lat || 
            this.attributes.center.lng !== center.lng){
...
var mapM = new MapM({
    center: L.latLng(RCData.initial.centerLat, RCData.initial.centerLng),
...
var InitiativeM = Backbone.Model.extend({
    ...
    // updateInVisibleBounds: function(){

    //     var coordinates = this.attributes.geoJson.geometry.coordinates;
    //     var latLng = L.latLng(coordinates[1], coordinates[0]);

    //     // TODO: what is the difference between "layer point" and "container point"?
    //     //var point = mapM.attributes.leafletMap.latLngToLayerPoint(latLng);
    //     var point = mapM.attributes.leafletMap.latLngToContainerPoint(latLng);
    //     if()

    //     if(point.x > RC.menuWidth && point.x < RC.mapWidth &&
    //         point.y > RC.navHeight && point.y < RC.mapHeight){
