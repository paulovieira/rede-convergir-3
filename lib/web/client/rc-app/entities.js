...
//    geoJsonLayersC: new OverlappingLayersC(),
    center: {
        lat: 39.5676,
        lng: -8.7068
    },
...
_.each(RCData.initiatives, function(obj){
    ...
    var type = RCData.definitions[obj.typeId];
    if(type){
        obj.typeTitle =  type.title.pt;
        obj.typeDescription =  type.description.pt;
    }
    else{
        obj.typeTitle =  obj.typeOther || "(no type)";
        obj.typeDescription =  "";
    }
...
var initiativesC = new InitiativesC(RCData.initiatives);

// create the geoJson for each initiative
initiativesC.each(function(model){
    ...
    model.set("geoJson", {
        ...
        "geometry": {
            "type": "Point",
            "coordinates": [model.get("coordinates")[1], model.get("coordinates")[0]]
        }
    });
