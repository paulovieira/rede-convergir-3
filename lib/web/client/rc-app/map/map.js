...
var MapIV = Mn.ItemView.extend({
    ...
    initializeMap: function(){
        ...
        this.map.on("moveend", _.throttle(this.updateModel, 10), this);

        this.map.on("moveend", function(){
            RC.mapIsMoving = false;
            /*
            var coordinates, point, model;
            for(var i=0, l=initiativesC.models.length; i<l; i++){
                ...
                point = this.map.latLngToContainerPoint(L.latLng(coordinates[1], coordinates[0]));
debugger;
                // update tge attribute in the model; we don't use .set for performance reasons
