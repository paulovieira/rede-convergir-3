...
var MenuBodyLV = Mn.LayoutView.extend({
    ...
    initialize: function(){
        ...
        this.getElementPosition = _.bind(this.getElementPosition, this);
        $(window).on("resize", this.getElementPosition);
    },
    ...
    onBeforeDestroy: function(){
        ...
        $(window).off("resize", this.getElementPosition);
    },

    onAttach: function(){

        // initialize the iCheck plugin 
        this.$("#options-filters-form").iCheck({
        ...
        // debugger;
        // RC.topLeftLatLng     = leafletMap.containerPointToLatLng([RC.menuWidth, 0]);
        // RC.bottomRightLatLng = leafletMap.containerPointToLatLng([RC.mapWidth, RC.mapHeight]);
        // RC.bounds            = L.latLngBounds(topLeftLatLng, bottomRightLatLng);
        // debugger;
    },
    ...
    getElementPosition: function(){
        //debugger;
        console.log("getElementPosition @ " + Date.now())

        // the width of the main container for the menu
        var menuWidth = this.$el.closest(".leaflet-control").width();

        // the manual padding added to the menu
        var padding = 5;

        RC.menuWidth = padding + menuWidth;
        RC.mapHeight = this.$el.closest(".leaflet-container").height();
        RC.mapWidth  = this.$el.closest(".leaflet-container").width();
        /*
        */
    },
    ...
    showExportMapModal: function(){
        ...
        var html = Marionette.Renderer.render("menu/templates/export-map.html", {
            ...
            baseLayer: mapM.get("baseLayersC").selected.get("id"),
            types: types,
            domains: domains,
            host: host
        });
    ...
    showList: function(){
        ...
        var numFiltered = initiativesC.where({visible: true}).length;
...
//debugger;
        ...
        var baseMapId = this.$("input[type='radio'][name='base-maps']")
                        .parent(".checked")
                        .children("input")
                        .val();
...
var MenuLV = ControlLV.extend({
    ...
    show: function(){
        ...
        var model = new Backbone.Model({
            "baseLayers": mapM.get("baseLayersC").toJSON(),
            "types": typesC.toJSON(),
