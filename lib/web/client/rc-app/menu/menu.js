...
var MenuBodyLV = Mn.LayoutView.extend({
    ...
    updateCollections: function(){
        ...
        var model, typeId, domains;
        var hasType, hasDomain;
        for(i=0, l=initiativesC.models.length; i<l; i++){

            typeId = initiativesC.models[i].attributes.typeId;    
            hasType = selectedTypes.length === 0 ? true : _.contains(selectedTypes, typeId);

            // optimized version - to be used if the performance is slow
            /*
            hasType = selectedTypes.length===0 ? true : false;
            if(!hasType){
                typeId = initiativesC.models[i].attributes.typeId;    

                // check if typeId is one of the selected types
                for(j=0, l2=selectedTypes.length; j<l2; j++){
            ...
            */
            ...
            domains = initiativesC.models[i].attributes.domains;
            hasDomain = selectedDomains.length === 0 ? true : (_.intersection(selectedDomains, domains).length > 0);
            
            initiativesC.models[i].set("inFilters", hasType && hasDomain);
        }
